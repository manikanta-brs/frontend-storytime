...
const AuthorStoriesPage = () => {
  ...
  // Add a fallback for location.state
  ...
  useEffect(() => {
    if (!location.state) {
      navigate("/authors"); // Redirect to the authors page
    }
  ...
  useEffect(() => {
    if (stories && stories.shows && stories.shows.items) {
  ...
  // If location.state is missing, return null (redirect will happen in useEffect)
  if (!location.state) {
    return null;
  }
