...
const SidebarPlayer = ({}) => {
  ...
  const { data = null, isLoading = false } = useGetEpisodesByShowIdQuery(
    storyInfo.id
  );
  ...
  useEffect(() => {
    if (data?.items) {
      ...
      if (list.length > 0) {
        ...
        setEpisodeDuration(convertMsToMinutesSeconds(list[0].duration_ms));
      }
    }
  }, [data]);
  ...
  useEffect(() => {
    ...
    // Pause the current audio and clean up
    ...
    // Set the progress if available
    if (episodeProgress) {
      audioRef.current.currentTime = episodeProgress;
    }

    // Play the audio if ready and playing
    if (isPlaying) {
